...
public class PhoneBookManager {
	...
	public static PhoneBookManager gerInstance() {
		return manager;
	}
...
//      System.out.println("입력하고자 하는 번호를 입력해주세요");
		...
		while (true) {
			...
			try {
				...
				if (!(select >= 1 && select <= 3)) {
					...
					BadNumberExeption e = new BadNumberExeption("잘못 된 메뉴 번호 입력");
		...
		while (true) {
			...
			try {
				if (name.isEmpty() || phoneNumber.trim().isEmpty() || addr.trim().isEmpty() || email.trim().isEmpty()) {
					StringEmptyException e = new StringEmptyException();
					throw e;
				}

			} catch (StringEmptyException e) {
				System.out.println("기본 정보는 공백없이 모두 입력 해 주셔야 합니다.");
