...
public class PhoneBookManager {
	...
	void addInfo(PhoneInfo info) {
		// 배열에 numOfInfo 숫자의 정수가 입력 할 때 -> index로 참조 값을 저장 해 준다. 
		books[numOfInfo] = info;
		// 입력 된 정보의 개수를 +1 증가 시킨다.
		numOfInfo++;
	}
	...
	void createInfo() {
		...
		// 입력 받기 위해서는 Scanner객체가 필요하다.
		System.out.println("입력하고자 하는 번호를 입력 해 주세요.");
		...
		// 2-1 기본 정보 수집 : 이름, 전화번호, 주소, 이메일
		System.out.println("이름을(를) 입력 해 주세요.");
		String name = kb.nextLine();
		
		System.out.println("전화번호을(를) 입력 해 주세요.");
		String phoneNumber = kb.nextLine();
		
		System.out.println("주소를(를) 입력 해 주세요.");
		String addr = kb.nextLine();
		
		System.out.println("이메일을(를) 입력 해 주세요.");
		String email = kb.nextLine();
				...
				case 1:
					...
					break;
				case 2:
					System.out.println("전공(학과)를 입력 해 주세요.");
					String major = kb.nextLine();
					System.out.println("학년 정보를 입력 해 주세요.");
					...
					break;
				case 3:
					System.out.println("회사의 이름을 입력 해 주세요.");
					String company = kb.nextLine();
					System.out.println("부서의 이름을 입력 해 주세요.");
					String dept = kb.nextLine();
					System.out.println("직무(직급)정보를 입력 해 주세요.");
	...
	int searchIndex(String name) {
		...
		int searchIndex = -1; // 정상적인 index값은 0이상의 값이 나온다.
							  // 찾지 못 했을 때 구분 값은 -1을 사용한다.
	
		// 배열의 반복으로 name 값을 비교해서 index값을 찾는다.
		for(int i=0; i<numOfInfo; i++) {
	...
	void showInfo() {
		...
		System.out.println("검색하실 이름을 입력 해 주세요.");
		String name = kb.nextLine();
	...
	// 5. 배열의 정보를 삭제 : 이름을 기준
	void deleteInfo() {
		System.out.println("삭제하고자 하는 이름을 입력 해 주세요.");
		String name = kb.nextLine();
	...
	void editInfo() {
		...
		System.out.println("변경하고자 하는 이름을 입력 해 주세요.");
		String name = kb.nextLine();
		...
		if(index<0) {
			...
			if(books[index] instanceof PhoneUnivInfo) {
				...
				System.out.println("회사 이름을 입력 해 주세요.");
				String company = kb.nextLine();
				System.out.println("부서 이름을 입력 해 주세요.");
				String dept = kb.nextLine();
				System.out.println("직급을 입력 해 주세요.");
				String job = kb.nextLine();
				...
				System.out.println("동호회 이름을 입력 해 주세요.");
				String cafeName = kb.nextLine();
				System.out.println("닉네임을 입력 해 주세요.");
				String nickName = kb.nextLine();
